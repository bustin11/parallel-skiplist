...
**Head**   | The beginning node. Often labeled as a dummy or sentinel node helperful for inerrstion and deletion.
...
## Deadlock
A situation in a multi-threaded program where 2+ threads are waiting for *each other's* resource (in this, a lock), and no system-wide progress can be made. Here is a fun analogy: Imagine there are two treasure chests, and two keys that unlock each treasure chest. Now if Chest A is locked with Key B, and Chest B is locked with Key A, then no one can open either chests. 
...
Livelock is difficult to identify because it's naturally sneaky and deceptive. Because we ensure that all locks are aquired in an acsending manner, we know that if a thread fails grabbing a lock, then another thread must hold that lock. But if another thread is holding that lock, then either that thread is waiting for another lock, has reached the highest level (so *it's* the holder of that lock). Then we know there is system-wide progress.

(Insert Image, cars moving back and forth)

## Starvation
Starvation is a matter of fairness. It's a situation where one or more threads are lacking progress for the vitality or progress of other threads. Going back to the Livelock analogy, we can think of just 2 cars driving oncoming and paralell to each, while the other 2 cars are always waiting, stopped at the light, watching the cars drive by. 

(Insert image of cars moving one way)
...
Here, I will go through what I did for each interface of the skiplist. I will also go through the design process as well as the struggles that lead me my solutions. 
...
I decided to go for a *Wait-Free* approach for searching because Searching is a read-only operation. Therefore, if I could avoid conjuring read-locks than I could make the concurrent skiplist faster since insertion and deletion rely on search operations. A Wait-Free (or Lock-free) algorithm is one where there is *per-thread* progress. This is subtly different from *system-wide* progress. In per-thread, we know everyone is making progress, while in system-wide, we know at least 1 thread is making progress. 

(Insert Image, actually don't need image)
...
Afterwards, we store the predesccors and successors to be used later on to ensure the local data structure while searching hasn't changed
...
I wrote some extensive test cases (see (Insert image directory here)), but even with the bugs, I still had to find obscure bugs, some of which would only be available in languages that do NOT do memory management for you.
...
That's when I decided to expliciting create a vector that would enumerate over all locks that I know for sure we acquired, and fortunately, it worked.
